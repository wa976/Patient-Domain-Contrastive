...
def parse_args():
    ...
    parser.add_argument('--data_folder', type=str, default='./data/total/fold1/')
...
def set_loader(args):
    if args.dataset == 'icbhi':        
        ...
        # unique_patient_ids_count = len(set(dataset.patient_ids))
        # print("patient num :",unique_patient_ids_count)
...
def train(train_loader, model, classifier, projector, criterion, optimizer, epoch, args, scaler=None):
    ...
    for idx, (images, labels) in enumerate(train_loader):
        ...
        bsz = class_labels.shape[0] if args.adversarial_ft or args.method == 'colar' or args.method == 'pdc' else labels.shape[0]
        ...
        [acc1], _ = accuracy(output[:bsz], class_labels if args.adversarial_ft or args.method =='colar' or args.method =='pdc' else labels, topk=(1,))
        # 바꿔야하는지 불확실
...
def main():
    ...
    if not args.eval:
        ...
        with torch.no_grad():
            for image, label_domain in train_loader:
                ori_image = image.cuda()
                new_image = image.reshape(image.shape[0], -1)
                new_image = new_image.cuda()
                label = label_domain[0].cuda()
                domain = label_domain[1].cuda()
                patient = label_domain[2].cuda()
                ...
                images.extend(new_image.cpu().numpy())
                patients.extend(patient.cpu().numpy())
                
                # probabilities = softmax(output)
              
                # for i in range(probabilities.shape[0]):
                #     print(f"Sample {i} Class Probabilities: {probabilities[i].cpu().numpy()}")
        ...
        #     idx = labels == label
        #     plt.scatter(reduced_images[idx, 0], reduced_images[idx, 1], color=label_colors[label], label=label, alpha=0.5)

        # plt.colorbar(format='%+2.0f dB')
        # plt.title('t-SNE Visualization train data before Training with class label')
        # plt.xlabel('t-SNE feature 1')
        # plt.ylabel('t-SNE feature 2')
        # plt.legend()
        # plt.savefig('./visualization/total/fold1_train_before_class.png', format='png', dpi=300, bbox_inches='tight')
        # plt.show()

        # plt.figure(figsize=(12, 8))

        # Create a color map for domain labels
        unique_domains = np.unique(domains)
        colors = plt.cm.viridis(np.linspace(0, 1, len(unique_domains)))
        domain_colors = dict(zip(unique_domains, colors))

        # for domain in unique_domains:
        #     idx = domains == domain
        #     plt.scatter(reduced_images[idx, 0], reduced_images[idx, 1], color=domain_colors[domain], label=domain, alpha=0.5)

        # plt.colorbar(format='%+2.0f dB')
        # plt.title('t-SNE Visualization train data before Training with domain label')
        # plt.xlabel('t-SNE feature 1')
        # plt.ylabel('t-SNE feature 2')
        # plt.legend()
        # plt.savefig('./visualization/total/fold1_train_before_domain.png', format='png', dpi=300, bbox_inches='tight')
        # plt.show()
        ...
        selected_patients = np.random.choice(unique_patients, size=10, replace=False)
        patients_colors = dict(zip(selected_patients, plt.cm.viridis(np.linspace(0, 1, len(selected_patients)))))


        # for patient in selected_patients:
        #     idx = patients == patient
        #     plt.scatter(reduced_images[idx, 0], reduced_images[idx, 1], color=patients_colors[patient], label=patient, alpha=0.5)

        # plt.colorbar(format='%+2.0f dB')
        # plt.title('t-SNE Visualization train data before Training with patient label')
        # plt.xlabel('t-SNE feature 1')
        # plt.ylabel('t-SNE feature 2')
        # plt.legend()
        # plt.savefig('./visualization/total/fold1_train_before_patient.png', format='png', dpi=300, bbox_inches='tight')
        # plt.show()
        ...
        for label in unique_labels:
            idx = labels == label
            plt.scatter(reduced_features[idx, 0], reduced_features[idx, 1], color=label_colors[label], label=label, alpha=0.5)

        plt.colorbar(format='%+2.0f dB')
        plt.title('t-SNE Visualization train data after Training with class label')
        plt.xlabel('t-SNE feature 1')
        plt.ylabel('t-SNE feature 2')
        plt.legend()
        plt.savefig('./visualization/total/fold1_train_after_dat_class.png', format='png', dpi=300, bbox_inches='tight')
        plt.show()
        ...
        for domain in unique_domains:
            idx = domains == domain
            plt.scatter(reduced_features[idx, 0], reduced_features[idx, 1], color=domain_colors[domain], label=domain, alpha=0.5)

        plt.colorbar(format='%+2.0f dB')
        plt.title('t-SNE Visualization train data after Training with domain label')
        plt.xlabel('t-SNE feature 1')
        plt.ylabel('t-SNE feature 2')
        plt.legend()
        plt.savefig('./visualization/total/fold1_train_after_dat_domain.png', format='png', dpi=300, bbox_inches='tight')
        plt.show()
        ...
        for patient in selected_patients:
            idx = patients == patient
            plt.scatter(reduced_features[idx, 0], reduced_features[idx, 1], color=patients_colors[patient], label=patient, alpha=0.5)

        plt.colorbar(format='%+2.0f dB')
        plt.title('t-SNE Visualization train data after Training with patient label')
        plt.xlabel('t-SNE feature 1')
        plt.ylabel('t-SNE feature 2')
        plt.legend()
        plt.savefig('./visualization/total/fold1_train_after_dat_patient.png', format='png', dpi=300, bbox_inches='tight')
        plt.show()
        ...
        patients =[]
        images = []
        softmax = nn.Softmax(dim=1)  # Softmax layer for converting logits to probabilities


        with torch.no_grad():
            for image, label_domain in val_loader:
                ori_image = image.cuda()
                new_image = image.reshape(image.shape[0], -1)
                new_image = new_image.cuda()
                label = label_domain[0].cuda()
        ...
        # plt.colorbar(format='%+2.0f dB')
        # plt.title('t-SNE Visualization val data before Training with class label')
        # plt.xlabel('t-SNE feature 1')
        # plt.ylabel('t-SNE feature 2')
        # plt.legend()
        # plt.savefig('./visualization/total/fold1_val_before_class.png', format='png', dpi=300, bbox_inches='tight')
        # plt.show()
        ...
        # plt.colorbar(format='%+2.0f dB')
        # plt.title('t-SNE Visualization val data before Training with domain label')
        # plt.xlabel('t-SNE feature 1')
        # plt.ylabel('t-SNE feature 2')
        # plt.legend()
        # plt.savefig('./visualization/total/fold1_val_before_domain.png', format='png', dpi=300, bbox_inches='tight')
        # plt.show()
        ...
        # plt.colorbar(format='%+2.0f dB')
        # plt.title('t-SNE Visualization val data before Training with patient label')
        # plt.xlabel('t-SNE feature 1')
        # plt.ylabel('t-SNE feature 2')
        # plt.legend()
        # plt.savefig('./visualization/total/fold1_val_before_patient.png', format='png', dpi=300, bbox_inches='tight')
        # plt.show()
        ...
        plt.colorbar(format='%+2.0f dB')
        plt.title('t-SNE Visualization val data after Training with class label')
        plt.xlabel('t-SNE feature 1')
        plt.ylabel('t-SNE feature 2')
        plt.legend()
        plt.savefig('./visualization/total/fold1_val_after_dat_class.png', format='png', dpi=300, bbox_inches='tight')
        plt.show()
        ...
        plt.colorbar(format='%+2.0f dB')
        plt.title('t-SNE Visualization val data after Training with domain label')
        plt.xlabel('t-SNE feature 1')
        plt.ylabel('t-SNE feature 2')
        plt.legend()
        plt.savefig('./visualization/total/fold1_val_after_dat_domain.png', format='png', dpi=300, bbox_inches='tight')
        plt.show()
        ...
        plt.colorbar(format='%+2.0f dB')
        plt.title('t-SNE Visualization val data after Training with patient label')
        plt.xlabel('t-SNE feature 1')
        plt.ylabel('t-SNE feature 2')
        plt.legend()
        plt.savefig('./visualization/total/fold1_val_after_dat_patient.png', format='png', dpi=300, bbox_inches='tight')
        plt.show()
